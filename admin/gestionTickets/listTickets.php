    ...
    <tbody>
        <?php foreach ($tickets as $ticket) : ?>
			...
			?>"
                ...
                <td>
                    <div class="actions">
                        <a href="#" onclick="toggleForm(<?= $ticket['id'] ?>)" class="actionButton" id="changeState" style="margin-left: 50px;">Changer État</a>
                        <a href="#" onclick="toggleFormAssign(<?= $ticket['id'] ?>)" class="actionButton" id="assignTicket" style="margin-left: 50px;">Assigner le ticket</a>
                    </div>
                </td>
            </tr>
        <?php endforeach; ?>
...
if (isset($_POST['assignerTicket'])) {
    $id = $_POST['ticketIdAssign'];
    $technicienEmail = $_POST['technicien'];

    // Update the ticket in the database with the technician email
    $query = "UPDATE Tickets SET adminTkt = '$technicienEmail' WHERE id = $id";

    if ($conn->query($query) === TRUE) {
        // Display a success message to the user
        echo '<script>alert("Ticket assigné avec succès !");</script>';

        // Reload the page or update the ticket list
        // ...
    } else {
        // Display an error message to the user
        echo '<script>alert("Erreur lors de l\'assignation du ticket !");</script>';
    }
}
